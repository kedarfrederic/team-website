/* ═══════════════════════════════════════════════════════════════
   v2-assets.js — assets page motion.

   Ported from the inline <script> in team-july2026-multipager/assets.html.
   It was missed in the original port: the page's CSS came across as
   v2-assets.css but this never did, so the scroll-choreographed motion
   ...
   ═══════════════════════════════════════════════════════════════ */
(() => {
  const root = document.getElementById('libPanel'); if (!root || !window.TM) return;
  const s = window.TM.seq(root.querySelectorAll('.acard'), 'in', 90, 120);
  window.TM.replay(root, s.play, s.reset, .25);
})();
