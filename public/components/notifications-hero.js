/**
 * Notifications hero — animation + content cycling.
 *
 * Two systems share the same DOM:
 *  1) Original animate-in + parallax loop, lifted from
 *     siteamrollouts/team-nomorechaos/nmc-page.js (lines 16–98) and
 *     scoped to the `.nh-` prefix.
 *  2) NEW: per-slot independent content cycler. Each of the 10 slots
 *     swaps its icon + sender + message + time on its own random
 *     interval (7–12s), so the hero feels continuously alive without
 *     looking like a slot machine.
 *
 * The slot HTML stays empty server-side — content is hydrated on load.
 */
(function () {
  ...
  /* ============ CHANNEL ICONS ============ */
  // bg = badge colour, svg = inner SVG markup (24x24 viewBox).
  // First 10 lifted verbatim from nmc-index.html — rest are music-industry
  ...
  var CHANNELS = {
    ...
    apple: {
      bg: '#FB233B',
      svg: '<svg viewBox="0 0 24 24" fill="#fff"><path d="M23.997 6.124c0-.738-.065-1.47-.24-2.19-.317-1.31-1.062-2.31-2.18-3.043C21.003.517 20.373.285 19.7.164c-.517-.093-1.038-.135-1.564-.15-.04-.003-.083-.01-.124-.013H5.988c-.152.01-.303.017-.455.026C4.786.07 4.043.15 3.34.428 2.004.958 1.04 1.88.475 3.208c-.192.448-.292.925-.363 1.408-.056.392-.088.785-.1 1.18 0 .032-.007.062-.01.093v12.223c.01.14.017.283.027.424.05.815.154 1.624.497 2.373.65 1.42 1.738 2.353 3.234 2.81.42.127.856.187 1.293.228.555.053 1.11.06 1.667.06h11.03a12.5 12.5 0 0 0 1.57-.1c.822-.106 1.596-.35 2.295-.81a5.046 5.046 0 0 0 1.88-2.207c.184-.42.292-.87.366-1.324.13-.792.17-1.592.172-2.392.005-3.61.003-7.22.003-10.83zm-6.341 6.675c-.71-.005-1.42-.072-2.114-.227-.81-.18-1.57-.482-2.25-.962-.225-.16-.435-.34-.633-.526-.02.063-.012.115-.012.166 0 1.504.005 3.008-.004 4.512-.003.482-.06.96-.205 1.423a3.382 3.382 0 0 1-1.48 1.926 3.405 3.405 0 0 1-1.523.483 3.464 3.464 0 0 1-3.65-2.602 3.448 3.448 0 0 1 1.418-3.722c.747-.502 1.575-.7 2.473-.6.193.022.385.066.586.103 0-.067 0-.114 0-.16 0-1.79.003-3.578-.005-5.367-.002-.288.04-.566.224-.787.27-.323.62-.466 1.034-.443.34.018.66.144.978.215 2.063.456 4.117.92 6.182 1.366.235.05.475.075.71.108.163.022.327.036.49.05.16.014.32-.018.49-.044V6.27c0-1.61-.005-3.22.003-4.83.001-.43-.103-.77-.45-.957a1.39 1.39 0 0 0-.45-.135c-.03-.005-.063-.013-.094-.02H4.5c-.044.01-.087.022-.13.03a1.62 1.62 0 0 0-.487.16c-.317.183-.456.49-.456.875.002 1.687.001 3.374.001 5.062 0 .073 0 .145.01.213z"/></svg>'
    },
    youtube: {
      bg: '#FF0000',
  ...
  /* ============ NOTIFICATION POOL ============
   * Each entry: { channel, sender, message, time }
   * `channel` keys into CHANNELS above for icon + colour.
   */
  var POOL = [
    ...
    { channel: 'apple',     sender: 'Apple Music',           message: 'Editorial pitch deadline — 24h left',          time: '17m ago' },
  ...
  /* ============ HYDRATE: fill each slot with a random pool entry ============ */
  function buildIcon(entry) {
    var ch = CHANNELS[entry.channel] || CHANNELS.slack;
    return { bg: ch.bg, html: ch.svg };
  }
  function pickRandom(excludeChannel) {
    // Pick a pool entry, avoiding repeating the same channel back-to-back in a slot.
    var idx = Math.floor(Math.random() * POOL.length);
    if (excludeChannel && POOL[idx].channel === excludeChannel) {
  ...
  // Initial hydrate — pick a random entry per slot, no duplicate channels visible at once.
  var seen = {};
  ...
  /* ============ ANIMATE-IN (lifted from nmc-page.js) ============ */
  notifs.forEach(function (n, i) {
    setTimeout(function () { n.classList.add('nh-notif-in'); }, 400 + i * 120);
  });

  /* ============ SCROLL: fade out when hero leaves view ============ */
  var visible = true;
  function onScroll() {
    var rect = hero.getBoundingClientRect();
    var heroVisible = rect.bottom > 100;
    if (heroVisible && !visible) {
      visible = true;
      notifs.forEach(function (n, i) {
        setTimeout(function () { n.classList.add('nh-notif-in'); }, i * 80);
      });
    } else if (!heroVisible && visible) {
      visible = false;
  ...
  /* ============ MOUSE PARALLAX (lifted from nmc-page.js) ============ */
  ...
  /* ============ PER-SLOT INDEPENDENT CONTENT CYCLER ============
   * Each slot has its own random 7–12s timer. When it fires:
   *   1) fade out (remove .nh-notif-in)
   *   2) wait for the 0.5s opacity transition
   *   3) repaint with a fresh pool entry (avoiding the current channel
   *      and any channel currently visible in another slot)
   *   4) fade back in
   *   5) schedule next swap
   */
  var FADE_MS = 500;
  var MIN_INTERVAL = 7000;
  var MAX_INTERVAL = 12000;

  function pickFreshFor(slot) {
    // Avoid the current slot's channel and any channel currently shown elsewhere.
    var inUse = {};
    notifs.forEach(function (n) {
      if (n !== slot && n.dataset.channel) inUse[n.dataset.channel] = true;
    });
    inUse[slot.dataset.channel] = true;
    // Try a handful of random picks; fall back to first non-conflicting entry.
    for (var i = 0; i < 12; i++) {
  ...
  function scheduleSwap(slot) {
    ...
    setTimeout(function () {
      // If the hero is currently scrolled out of view, defer — re-check soon.
      if (!visible) {
        scheduleSwap(slot);
        return;
      }
      ...
      setTimeout(function () {
        ...
        // Force reflow before re-adding the class so the transition replays.
        // eslint-disable-next-line no-unused-expressions
        slot.offsetHeight;
        slot.classList.add('nh-notif-in');
        scheduleSwap(slot);
      }, FADE_MS + 20);
    }, delay);
  }

  // Stagger the first swap per slot so they don't all fire at once even
  // if the random rolls happen to cluster.
